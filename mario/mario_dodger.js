...
const startGame = () => {
  ...
  const timeUp = window.setInterval(() => {
...
const collision = () => {
  ...
  if (rock.offsetTop >= limit) {
    if (
      (rock.offsetLeft <= dodger.offsetLeft &&
        rockRight >= dodger.offsetLeft) ||
...
const finishGame = (intervalId, timeUp, userScoreSpan) => {
  if (userScoreSpan.innerText > highScore.innerText) {
    highScore.innerText = userScoreSpan.innerText;
  }
  createDivAlert();
